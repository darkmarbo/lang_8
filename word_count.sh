...
if(($#<2));then
    echo "usage: $0 text_dir charset_list dict_num "
    echo "example: $0 text en-gb.list 3 "
    exit 
fi
...
do
    ...
    python count.py $tmp_all $tmp_count $charset_list 
    if(($? != 0));then
        echo "count.py err!"
        exit
    fi
...
#################### 词典 进行合并   ############################### 
python count_mix.py $out_dir_dict ${out_dir_dict}.all
if(($? != 0));then
...
python count_mix.py temp_dict ${text_dir}.allword
if(($? != 0));then
