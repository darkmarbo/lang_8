...
file_del_fy = file_name + "_del_cons"
...
for line in fp_charset:
    try:
        ...
        print(e);
        print("decode utf-8 err");
        vec_line=[];
    if len(vec_line) < 3:
        print ("split charset err!")
        continue;
...
for line in fp_in:
    ...
    try:
        ...
        print(e);
        print("split input_line err");
        #list_line = [];

    print "list_line:"
    print list_line;
    num_test = len(list_line);
    if num_test < 2:
        print "list_line222:"
        print list_line;
        print ("split line err! split_num_line=%d"%(num_test))
        fp_err.write("%s\n"%(line));
        continue;
    ...
    ###  a b c d 
    if (1 == len(word) or word in char_list_fuyin  or word in char_list_yuanyin):
        fp_err.write("%s\n"%(line));
        continue;
